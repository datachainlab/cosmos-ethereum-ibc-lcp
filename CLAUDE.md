...
#### Polygon (tm2pol) — Phase 2a handshake

`cases/tm2pol/` exercises the full LCP-mediated client + connection + channel handshake between Cosmos (`ibc0`, simapp) and Polygon PoS (`ibc1`, kurtosis-pos bor). It does **not** yet run packet relay, ack drain, operators, or channel upgrade — those are deferred to later phases. The Polygon-side Solidity (`IBCHandler`, `LCPClientIAS`, `AppV1`) is deployed at devnet bring-up.
