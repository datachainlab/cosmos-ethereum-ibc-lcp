...
- **`tests/e2e/scripts/run_e2e_test_pol.sh`** — top-level driver invoked by `make e2e-test-pol`. Brings up LCP, exports `LCP_RA_ROOT_CERT_HEX` / `LCP_DCAP_RA_ROOT_CERT_HEX` if `SGX_MODE=SW`, runs `make -C cases/tm2pol network` (which brings up tendermint + polygon + deploys contracts), waits for heimdall `/status`, runs `setup → handshake → [test-channel-upgrade] → test → test-operators`, then tears LCP and the devnets down. Parses `--upgrade_test` from `E2E_OPTIONS`, which sets `USE_UPGRADE_TEST=yes` (flowed through to the contracts compose service so `deploy.js` also deploys + proposes `AppV2`-`AppV7`). `make e2e-test-pol` depends on `e2e-clean-pol` which wipes polygon's `.openzeppelin/` upgrade manifest and tears down any leftover tendermint/polygon networks from a prior failed run.
