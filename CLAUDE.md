...
#### Polygon (tm2pol) — Phase 2b handshake + packet relay

`cases/tm2pol/` exercises the full LCP-mediated client + connection + channel handshake between Cosmos (`ibc0`, simapp) and Polygon PoS (`ibc1`, kurtosis-pos bor), plus bidirectional packet relay and ack drain. It does **not** yet run operators or channel upgrade — those are deferred to Phase 2c. The Polygon-side Solidity (`IBCHandler`, `LCPClientIAS`, `AppV1`) is deployed at devnet bring-up. `foundry`'s `cast` must be on `$PATH` (used by `scripts/utils:sendPacketOnPol` to send the bor-side mockapp packet).
...
- **`chains/polygon/contracts/`** — self-contained hardhat project (separate from `chains/ethereum/contracts/`). `package.json` pins yui-ibc-solidity v0.3.40, lcp-solidity v0.2.1, ethereum-ibc-relay-chain v0.3.21. `Dependencies.sol` is IAS-only — no risc0/DCAP imports — so no foundry submodule is needed. `hardhat.config.js` `bor_local` network reads `process.env.BOR_RPC_URL` (kurtosis port forwarding is dynamic) and uses the kurtosis admin private key `0xd403…60ea` as the deployer (only L2-prefunded account). `scripts/deploy.js` deploys IBCHandler + LCPClientIAS + AppV1 only, and funds the relayer's HD-signer EOA (`0xa89F47…`) with 100 ETH out of the admin so the HD-signer relayer can pay gas — the mnemonic-derived address isn't prefunded by kurtosis-pos. The contracts compose service uses `network_mode: host` to reach kurtosis's 127.0.0.1:NNNNN bor RPC port.
- **`tests/e2e/scripts/run_e2e_test_pol.sh`** — top-level driver invoked by `make e2e-test-pol`. Brings up LCP, exports `LCP_RA_ROOT_CERT_HEX` / `LCP_DCAP_RA_ROOT_CERT_HEX` if `SGX_MODE=SW`, runs `make -C cases/tm2pol network` (which brings up tendermint + polygon + deploys contracts), waits for heimdall `/status`, runs `setup → handshake → test`, then tears LCP and the devnets down. `make e2e-test-pol` depends on `e2e-clean-pol` which wipes polygon's `.openzeppelin/` upgrade manifest and tears down any leftover tendermint/polygon networks from a prior failed run.
