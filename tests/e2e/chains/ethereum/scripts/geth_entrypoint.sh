...
# Currently: amsterdamTime for Glamsterdam/Gloas (update when new HF is added)
...
if [ "$EPOCH_LATEST_HF" -gt 0 ]; then
    ...
    sed -i.bak s/"\"amsterdamTime\": 0/\"amsterdamTime\": ${LATEST_HF_TIMESTAMP}/" /execution/genesis.json
fi
